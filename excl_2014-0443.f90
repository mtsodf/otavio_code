...
!write(6, '(F12.8, X, F12.8, X, F12.8, X, F12.8)') tax_rate, min_output, max_debt_min_revenue, b_sup
...
DOUBLE PRECISION, DIMENSION(b_num_short, b_num_long,  y_num, 2) :: v0_matrix_new, v1_matrix_new, q_nodef_matrix_new, &
                                                                   v_excl_matrix_new, tax0_matrix_new, tax1_matrix_new, output0_matrix, output1_matrix, &
...
!    call read_q_ev   !READ MATRICES OF SPLINE COEFFICIENTS TO APPROXIMATE EXPECTED VALUE FUNCTIONS AND BOND PRICE FUNCTION
           ...
           ! WRITE(nout, *) i_y
               ...
               do i_b_short =  1, b_num_short
                   ...
                   do i_b_long = 1, b_num_long
                      ...
                      v_excl_matrix_new(i_b_short, i_b_long, i_y_global, i_access) = v_valor
         ...
         do i_b_long = 1,b_num_long
             do i_y = 1,y_num
                do i_access = 1,access_num
                     ...
                     indicator_tirar=0
                     WRITE(10, '(F20.10, X, F20.10, X, F20.10, X, F20.10)') v_matrix_new(i_b_short, i_b_long, i_y, i_access), &
 ...
 !           WRITE(nout, '(F14.8, X, F14.8, X, F14.8, X, F14.8, X, F14.8, X, F14.8)') g_initial-b_short_initial, tax, tfp, output, consum, v1_matrix(i_b_short, 1, i_y, i_access)
     ...
     do i_b_short = 1,b_num_short
         do i_b_long = 1,b_num_long
             do i_y = 1,y_num
                do i_access = 1,access_num
                     read(10, '(F20.10, X, F20.10, X, F20.10, X, F20.10)') v_matrix(i_b_short, i_b_long, i_y, i_access), &
